...
# Load data
df = pd.read_csv('part-00000.csv',sep=';')
            ...
            [
                html.Div(
                    [
                        ...
                        dcc.RadioItems(
                            id='manufacturer_status_selector',
                            options=[
                                {'label': 'All ', 'value': 'all'},
                                {'label': 'Active only ', 'value': 'active'},
                            ],
            ...
            [
                html.Div(
                    [
                        dcc.Graph(
                            id='main_graph',
                            figure = px.line(dfff, x='date', y='GOOG'),
                        )
                    ],
...
def filter_dataframe(df, operator_options, manufacturer_options, start_date, end_date):
    dff = df[df['Operator'].isin(operator_options)
             & df['Manufacturer'].isin(manufacturer_options)
             & (df['Date'] >= start_date)
             & (df['Date'] <= end_date)]
    return dff
...
@app.callback(Output('nb_flights_graph', 'figure'),
              ...
              Input('manufacturer_dropdown', 'value'),
              Input('date_picker_range', 'start_date'),
              Input('date_picker_range', 'end_date')])
def make_main_figure(operator_selected, manufacturer_selected, start_date,end_date):

    print('----------------------- First callback -----------------------')
    dff = filter_dataframe(df, operator_selected, manufacturer_selected, start_date, end_date)
    df_graph = dff.groupby([df['Date']]).sum().reset_index()
    print(df_graph)
    fig = go.Figure()
...
@app.callback(Output('main_graph', 'figure'),
              ...
              Input('date_picker_range', 'start_date'),
              Input('date_picker_range', 'end_date')])
def make_weekday_figure(operator_selected, manufacturer_selected, start_date,end_date):

    print('---------------- im in the second callback --------------------')
    dff = filter_dataframe(df, operator_selected, manufacturer_selected, start_date, end_date)
    df_graph = dff.groupby([df['dayOfWeek']]).sum().reset_index()
    print(df_graph)
    fig = go.Figure()
